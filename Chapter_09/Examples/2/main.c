...
// uncomment for kernel oops
// #define ENABLE_OOPS 1
...
#endif
{
	...
	if (*eof) { n=0; }
	  ...
	  tt = inl(0x40); 		// units is 1/MHz .. uSec
		mdelay(thisSLEEP);	// units is 1ms i.e. 1/KHz
	  ...
	  ttr=(tt2>tt)?tt2-tt:((0xfffffffe)-tt)+tt2;			// prep for 64bit div
		...
		n=sprintf(buf, "Hello .. I got \"%s\":\n\t[ jiffies=%ld MHz (tt2=%lld; tt=%lld; j2=%ld; j1=%ld)].\n", 
				usrsp->proc_hello_value,ttr, tt2,tt,j2,j1);
