  ...
  unsigned long count, void *data)
{
  ...
  if (copy_from_user(usrsp->CDD_procvalue, buf, length)) return -EFAULT;
...
static int CDD_init(void)
{
  ...
  // Create the necessary proc entries
	for (i=0;i<CDDNUMDEVS; i++) {
		...
		thisCDD=&(CDD_dev[i]);
    if (!(thisCDD->devno)) continue;
		sprintf(CDD_procdata[i].CDD_procname,"%s%d",CDD,i);
		
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
  	proc_CDD2[i] = proc_create(CDD_procdata[i].CDD_procname, 
				0777, proc_myCDD, &proc_fops);
    CDD_procdata[i].data = (void *)i;
#else
    proc_CDD2[i] = create_proc_entry(CDD_procdata[i].CDD_procname,
			0,proc_myCDD);
...
static void CDD_exit(void)
{
	...
	for (i=0;i<CDDNUMDEVS; i++) {
		thisCDD=&(CDD_dev[i]);
    if (!thisCDD->devno) continue;
    if (proc_CDD2[i]) remove_proc_entry (CDD_procdata[i].CDD_procname, 
			proc_myCDD);
	}
